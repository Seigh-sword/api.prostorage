...
class Database {
    debug(log) {
        if (!this.tgdb.config.debug)
            return;
        console.log(`${chalk_1.dim.cyan(`[${new Date().toISOString()}]`)} \
[${(0, chalk_1.blue)("TGDB")}@${(0, chalk_1.green)(this.name)}] ${log}`);
    }
    constructor(tgdb, name, entryPoint) {
        this.tgdb = tgdb;
        this.name = name;
        this.entryPoint = entryPoint;
    }
    async getMessage(messageId) {
        const messages = await this.tgdb.client.getMessages(this.tgdb.config.channelId, { ids: messageId });
        return messages[0];
    }
    async sendMessage(text) {
        return await this.tgdb.client.sendMessage(this.tgdb.config.channelId, { message: text });
    }
    async editMessage(messageId, text) {
        return await this.tgdb.client.editMessage(this.tgdb.config.channelId, { message: messageId, text: text });
    }
    async deleteMessage(messageId) {
        return await this.tgdb.client.deleteMessages(this.tgdb.config.channelId, [messageId], { revoke: true });
    }
    async getRecordsIndexes(firstIndexId = this.entryPoint) {
    ...
    async getRecordValue(messageId) {
        const { text } = await this.getMessage(messageId);
        const { header, value } = (0, parse_1.parseData)(text);
        let record = value;
    ...
    async add(key, value) {
        var _a, _b, _c, _d;
        const clean = (0, utils_1.isClean)(key);
        if (!clean) {
            throw new Error(`Invalid key '${key}'. A key can only contain A-Z, a-z, 0-9, - and _`);
        }
        const indexes = await this.getRecordsIndexes();
        const records = {};
        for (const index of indexes) {
        ...
        const entryPointMsg = await this.sendMessage(`${key} 0 null null\n${this.name} ${this.entryPoint}\n${values[0]}`);
        ...
        if (modifiedText.length > VLPP) {
            const newIndex = await this.sendMessage(`${this.name} ${indexes.length} ${msgToEditId} null\n${key} ${entryPointMsg.id}`);
        ...
        if (values.length > 1) {
            for (let i = 1; i <= values.length; i++) {
                ...
                if (!value)
                    continue;
                const prevMsg = await this.sendMessage(`${key} ${i} ${(_b = sentMsgs.at(-1)) === null || _b === void 0 ? void 0 : _b.id} null
${this.name} ${this.entryPoint}\n${value}`);
...
${(_c = sentMsgs.at(-1)) === null || _c === void 0 ? void 0 : _c.text.split("\n").slice(1).join("\n")}`;
                await this.editMessage((_d = sentMsgs.at(-1)) === null || _d === void 0 ? void 0 : _d.id, modifiedText);
    ...
    async edit(key, value) {
        ...
        const newValues = []; // new values for each page
        ...
        if (newPageCount > oldPageCount) {
            ...
            for (let i = 0; i < pagesToAdd; i++) {
                const { id } = await this.sendMessage(`${key} ${oldPageCount + i} ${oldMsgIds.at(-1)} null
${this.name} ${this.entryPoint}\n${newValues[oldPageCount + i]}`);
    ...
    async delete(key) {
        ...
        for (let i = 0; i < recordIndexes.length; i++) {
            ...
            const indexMsgId = (_b = (_a = recordIndexes[i - 1]) === null || _a === void 0 ? void 0 : _a.header.next_msg_id) !== null && _b !== void 0 ? _b : this.entryPoint;
            if (key in table) {
                ...
                const text = `${this.name} ${header.page_index} ${header.prev_msg_id} ${header.next_msg_id}\n${records}`;
                await msg.edit({ text });
                break;
            }
        }
