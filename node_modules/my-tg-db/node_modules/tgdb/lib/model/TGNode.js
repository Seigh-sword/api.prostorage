...
/**
 * Get all edges based on direction, return all if no direction specified.
 * @param edgeDirection
 */
TGNode.prototype.getEdges = function(edgeDirection) {
	
	logger.logDebugWire('Get edge : total size = ' + this._edges.length);
    var edges    = [];
    for (var i = 0; i < this._edges.length; i++) {
        if (!edgeDirection||(this._edges[i].getDirection()===edgeDirection)) {
            edges.push(this._edges[i]);
        }
    }
	logger.logDebugWire('Get edge : returned size = ' + edges.length);
    return edges;
};
...
TGNode.prototype.addEdge = function(edge) {
	if(!edge) {
		logger.logDebugWire('Add edge : Edge undefined !!!!!');
	} else {
		logger.logDebugWire('Add edge : ' + edge);
	}
