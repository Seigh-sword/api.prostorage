...
CommitTransactionResponse.prototype.processTransactionStatus = function (inputStream, status) {
    ...
    switch (ts) {
        ...
        /* Since 1.1 */
        case TransactionStatus.TGTransactionUniqueIndexKeyAttributeNullError:
        default:
            try {
                msg = inputStream.readUTF();
            	logger.logDebugWire(
            			"[CommitTransactionResponse.prototype.processTransactionStatus] Transaction status : %s", 
            			msg);                
            }
            catch (Error) {
                msg = "Error not available";
            }
        	logger.logDebugWire("[CommitTransactionResponse::processTransactionStatus] status = %d, msg = %s", status, msg);
