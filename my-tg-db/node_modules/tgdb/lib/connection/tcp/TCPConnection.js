...
var util                      = require('util'),
    ...
    TGTransactionExceptions   = require('../../exception/TGTransactionException'),
...
TCPConnection.prototype.initMetadata = function () {
	if (!this._gof) {
		return;
	}
	var gmd = this._gof.getGraphMetaData();
	if (gmd.isInitialized()) {
		return;
	}
	
	this.getGraphMetadata(true);
};

/*
 *   Connection AIP
 */
TCPConnection.prototype.getGraphObjectFactory = function() {
	this.initMetadata();
	return this._gof
};
...
TCPConnection.prototype.handleGetGraphMetadataResponse = function(response) {		
	...
	var metadata = this._entities.updateGraphMetaData(attrDescList, nodeTypeList, edgeTypeList);
	var callback = response.getRequest().getCallback();
	if(!!callback) {
		callback(metadata);
	}
};
...
TCPConnection.prototype.executeQuery = function (queryExpr, queryOption, callback) {
	logger.logDebug( 'Entering TCPConnection.prototype.executeQuery .......... ');
//	this.initMetadata();
	
    var channel = this.getChannel();
	logger.logDebug("[TCPConnection::executeQuery] queryExpr = %s",  queryExpr);
	var request = ProtocolMessageFactory.createMessageFromVerbId(
		VerbId.QUERY_REQUEST, 
		channel.getAuthToken(), 
		channel.getSessionId(),
		callback
	);
	...
	if(!!queryOption) {
		request.setEdgeFilter(queryOption.getEdgeFilter());
		request.setTraversalCondition(queryOption.getTraversalCondition());
		request.setEndCondition(queryOption.getEndCondition());
	}
	channel.send(request);
...
TCPConnection.prototype.executeQueryWithId = function (queryHashId, queryOption, callback) {
	logger.logDebugWire( "[TCPConnection::executeQueryWithId] Entering TCPConnection.prototype.executeQueryWithId .......... ");
    ...
    try {
        ...
        this.getChannel().send(request);
        logger.logDebugWire( '[TCPConnection::executeQueryWithId] Send execute query with id completed');
    }
    catch (exception) {
        this.throwException(exception);
    }
...
//	request.setConnectionId(connId);
	request.setQueryHashId(queryHashId);
	this.getChannel().send(request);
	logger.logDebug( "[TCPConnection::closeQuery] Send close query completed");
};
...
function configureQueryRequest(queryRequest, properties) {
	...
	var fetchSize = properties.getPrefetchSize();
	if(!!fetchSize) {
	...
	var traversaldepth = properties.getTraversalDepth();
	if(!!traversaldepth) {
		queryRequest.setTraversalDepth(traversaldepth);
	}
	
	var tdepth = properties.getEdgeLimit();
	if(!!tdepth) {
		queryRequest.setEdgeFetchSize(tdepth);
	}
	
	var sortName = properties.getSortAttrName();
	if (!!sortName) {
		queryRequest.setSortAttrName(sortName);
	}

	var isDec = properties.isSortOrderDsc();
	if (null!==isDec&&'undefined'!==isDec) {
		queryRequest.setSortOrderDsc(isDec);
	}

	var limit = properties.getSortResultLimit();
	if (!!limit) {
        if (limit >=0) {
        	queryRequest.setSortResultLimit(limit);
        }
	}

	logger.logDebug("[TCPConnection::configureQueryRequest] fetchSize - " + fetchSize);
	logger.logDebug("[TCPConnection::configureQueryRequest] batchSize - " + batchSize);
	logger.logDebug("[TCPConnection::configureQueryRequest] traversaldepth - " + traversaldepth);
	logger.logDebug("[TCPConnection::configureQueryRequest] tdepth - " + tdepth);
	logger.logDebug("[TCPConnection::configureQueryRequest] sortName - " + sortName);
	logger.logDebug("[TCPConnection::configureQueryRequest] isDec - " + isDec);
	logger.logDebug("[TCPConnection::configureQueryRequest] limit - " + limit);
